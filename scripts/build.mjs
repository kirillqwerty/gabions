...
const mainJs = await readFile(path.join(source, 'assets/js/main.js'));
const mainJsVersion = createHash('sha256').update(mainJs).digest('hex').slice(0, 12);
...
for(const file of await walk(out)) if(/\.(html|xml|txt)$/.test(file)) {
  ...
  if (file.endsWith('.html')) output=output.replace(/(assets\/js\/main\.js)(?:\?v=[a-f\d]+)?"/g, `$1?v=${mainJsVersion}"`);
